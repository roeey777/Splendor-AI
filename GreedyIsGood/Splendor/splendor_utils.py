...
CARDS = {
    "1g1w1r1b": ("black", {"green": 1, "white": 1, "red": 1, "blue": 1}, 1, 0),
...
def convert_filename(filename):
    ...
    if f[
        -1
    ].isdigit():  # If last character is digit, this is a gem asset. Process accordingly.
        f = f.split("_")
        return (
            (f[0], int(f[1])) if len(f) == 2 else (f[0], int(f[2]))
        )  # Return colour and number.
    colour, code = (
        f.split("_") if f[0].isalpha() else (None, f)
    )  # Else, this is a card asset. Process accordingly.
...
class AgentTrace:
    def __init__(self, pid):
        ...
        self.action_reward = (
            []
        )  # Turn-by-turn history consisting of (action,reward) tuples.
...
def GemsToString(gem_dict):
    ...
    if len(gem_counts) == 1:
        return "{} {} gem{}".format(
            gem_counts[0][1], gem_counts[0][0], "s" if gem_counts[0][1] > 1 else ""
        )
    elif len(gem_counts) == 2:
        return "{} {} and {} {} gems".format(
            gem_counts[0][1], gem_counts[0][0], gem_counts[1][1], gem_counts[1][0]
        )
...
def ActionToString(agent_id, action):
    ...
    if "collect" in action["type"]:
        if len(action["returned_gems"]):
            desc = "Agent {} collected {}, exceeded the limit, and returned {}.".format(
                agent_id,
                GemsToString(action["collected_gems"]),
                GemsToString(action["returned_gems"]),
            )
        else:
            desc = "Agent {} collected {}.".format(
                agent_id, GemsToString(action["collected_gems"])
            )

    elif action["type"] == "reserve":
        desc = "Agent {} reserved a Tier {} {} card.".format(
            agent_id, card.deck_id + 1, card.colour
        )

    elif "buy" in action["type"]:
        if card.points:
            desc = "Agent {} bought a {}Tier {} {} card, earning {} point{}!".format(
                agent_id,
                "previously reserved " if "reserved" in action["type"] else "",
                card.deck_id + 1,
                card.colour,
                card.points,
                "s" if card.points > 1 else "",
            )
        else:
            desc = "Agent {} bought a {}Tier {} {} card.".format(
                agent_id,
                "previously reserved " if "reserved" in action["type"] else "",
                card.deck_id + 1,
                card.colour,
