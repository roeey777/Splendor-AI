...
class SplendorGameRule(GameRule):
    ...
    def generateSuccessor(self, state, action, agent_id):
        ...
        # Record last action such that other agents can make use of this information.
    ...
    def calScore(self, game_state, agent_id):
        ...
        if len(victors) > 1 and agent_id in [d[0] for d in victors]:
            ...
            if bought_cards == min_cards:
                # Add one point if this agent was a tied victor, and had fewer cards.
                return game_state.agents[agent_id].score + 1
    ...
    def resources_sufficient(self, agent, costs):
        ...
        for colour, cost in costs.items():
            ...
            # Shortfall shouldn't be negative.
            shortfall = max(cost - available, 0)
            wild -= shortfall
    ...
    def getLegalActions(self, game_state, agent_id):
        ...
        for card in board.dealt_list() + agent.cards['yellow']:
            ...
            returned_gems = self.resources_sufficient(agent, card.cost)
