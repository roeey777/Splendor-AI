...
def loadAgent(file_list, name_list, superQuiet=True):
    ...
    for i, agent_file_path in enumerate(file_list):
        ...
        try:
            ...
            print(
                'Error: Agent at "' + agent_file_path + '" could not be loaded!',
                file=sys.stderr,
            )
            traceback.print_exc()
            pass
...
def run(options, valid_game, msg):
    ...
    if options.replay != None:
        ...
        for game_num in range(options.multipleGames):
            ...
            agents, load_errs = loadAgent(
                filelist, agents_names, superQuiet=options.superQuiet
            )
            is_load_err = False
                ...
                for i in range(NUM_AGENTS):
                    print(
                        "    {} earned {:.2f} on average and won {} games ({:.2f})%.".format(
                            agents_names[i], avgs[i], wins[i], win_rates[i]
                        )
                    )
...
def loadParameter():
    ...
    parser.add_option(
        "-r", "--ruby", help="Ruby team agent file", default=DEFAULT_AGENT
    )
    parser.add_option(
        "-c", "--citrine", help="Citrine team agent file", default=DEFAULT_AGENT
    )
    ...
    parser.add_option(
        "--no-highlighting",
        action="store_true",
        help="Disables highlighting of potential card purchases",
        default=False,
    )
