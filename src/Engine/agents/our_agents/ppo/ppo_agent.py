...
from Engine.Splendor.gym.envs.utils import create_action_mapping, create_legal_actions_mask
...
class PPOAgent(Agent):
    ...
    def SelectAction(self, actions, game_state, game_rule):
        state: np.array = extract_metrics_with_cards(game_state, self.id).astype(np.float32)
