...
class AgentArea:
    def __init__(self, root, agent_id, agent_title):
        ...
        self.agent_title = make_label(
            root,
            textvariable=self.agent_title_var,
            x=NAME_POS[0],
            y=NAME_POS[1] + PLYR_SEP * agent_id,
            h=30 * s,
            w=300 * s,
            font=("times", fsize),
            bg="black",
    ...
    def set_agent_title(self, score):
        if len(self.title_text) <= 20:
            self.agent_title_var.set(
                "Agent #{}: {}. Score: {}".format(self.agent_id, self.title_text, score)
            )
        else:
    ...
    def update(self, agent, resources):
        ...
        for colour in COLOURS.values():
            ...
            if agent.gems[
                colour
            ]:  # Draw gem counter only if there are gems in the stack.
                gem = self.root.create_image(
                    xpos,
                    ypos,
                    image=resources["gems_small"][colour][agent.gems[colour]],
                    tags="gem_small",
                )
            ...
            for card in agent.cards[colour]:
                ...
                self.cards[colour][j] = self.root.create_image(
                    xpos,
                    ypos,
                    image=resources["cards_small"][card.colour][card.code],
                    tags="card_small",
                )
                if (
                    colour == "yellow"
                ):  # If these cards are yellow, that means they are unpaid. Display with card sleeve.
                    self.sleeves[j] = self.root.create_image(
                        xpos, ypos, image=resources["card_sleeve"], tags="card_small"
                    )
                j += 1
            i += 1
        ...
        for i in range(len(agent.nobles)):
            ...
            self.nobles.append(
                self.root.create_image(
                    xpos,
                    ypos,
                    image=resources["nobles_small"][agent.nobles[i][0]],
                    tags="noble_small",
                )
            )
...
class BoardArea:
    def __init__(self, root):
        ...
        self.cntrs = [None] * 3
        for i in range(3):
            self.cntrs[i] = self.root.create_text(
                (D_COUNTR[0], D_COUNTR[1] + CARD_SEP[1] * i),
                text=str([40, 30, 20][i]),
                font=("times", int(20 * s)),
                fill=["green", "#bf7c1d", "blue"][i],
            )
    ...
    def update(self, state, resources, no_highlighting):
        ...
        if self.start:
            ...
            for step in animation_sequence:
                for card in step:
                    ...
                    self.root.itemconfigure(
                        self.cntrs[deck_id], text=str(deck_counts[deck_id])
                    )
                    card = board.dealt[deck_id][loc]
                    xpos, ypos = (
                        CARD_POS[0] + CARD_SEP[0] * loc,
                        CARD_POS[1] + CARD_SEP[1] * deck_id,
                    )
                    self.dealt[deck_id][loc] = self.root.create_image(
                        xpos,
                        ypos,
                        image=resources["cards_large"][card.colour][card.code],
                        tags="card_large",
                    )
                colour = list(COLOURS.values())[i]
                self.gems[colour] = self.root.create_image(
                    GEMS_POS[0],
                    GEMS_POS[1] + GEMS_SEP * i,
                    image=resources["gems_large"][colour][board.gems[colour]],
                    tags="gem_large",
                )
                if i < len(board.nobles):
                    self.nobles.append(
                        self.root.create_image(
                            NOBL_POS[0],
                            NOBL_POS[1] + NOBL_SEP * i,
                            image=resources["nobles_large"][board.nobles[i][0]],
                            tags="noble_large",
                        )
                    )
                self.root.update()
        ...
        # Else, iterate over decks like normal.
            ...
            for i in range(3):
                ...
                for j in range(4):
                    xpos, ypos = (
                        CARD_POS[0] + CARD_SEP[0] * j,
                        CARD_POS[1] + CARD_SEP[1] * i,
                    )
                    if board.dealt[i][
                        j
                    ]:  # Generate image only if there is a card present at this position.
                        card = board.dealt[i][j]
                        self.dealt[i][j] = self.root.create_image(
                            xpos,
                            ypos,
                            image=resources["cards_large"][card.colour][card.code],
                            tags="card_large",
                        )
                        if not no_highlighting and not can_buy(
                            agent, board.dealt[i][j]
                        ):
                            self.dealt_transparencies[i][j] = self.root.create_image(
                                xpos,
                                ypos,
                                image=resources["card_dull"],
                                tags="card_large",
                            )
        ...
        for colour in COLOURS.values():
            ...
            if count:  # Draw gem counter only if there are gems in this stack.
                self.gems[colour] = self.root.create_image(
                    xpos,
                    ypos,
                    image=resources["gems_large"][colour][count],
                    tags="gem_large",
                )
            i += 1
...
class GUIDisplayer(Displayer):
    def __init__(self, half_scale, delay=0.1, no_highlighting=False):
        ...
        global s, PLYR_POS, NAME_POS, PGEM_POS, PCRD_POS, PNBL_POS, PCRD_SEP, PLYR_SEP, D_COUNTR, RUNNING, CARD_POS, CARD_SEP, GEMS_POS, GEMS_SEP, NOBL_POS, PNBL_SEP, NOBL_SEP, CNVS_DIM, ACTN_BOX
        ...
        PCRD_SEP = (
            49 * s,
            15 * s,
        )  # Horizontal and vertical separation between player cards.
        CARD_SEP = (
            220 * s,
            298 * s,
        )  # Horizontal and vertical separation between available cards.
    ...
    def InitDisplayer(self, runner):
        ...
        self.resources = {
            ...
            "gems_large": {c: {} for c in COLOURS.values()},
            "gems_small": {c: {} for c in COLOURS.values()},
            "nobles_large": {},
            "nobles_small": {},
        }
        ...
        self.table = self.canvas.create_image(
            0, 0, image=self.resources["background"], anchor="nw"
        )
        ...
        self.action_frame.place(
            x=ACTN_BOX[0], y=ACTN_BOX[1], height=510 * s, width=314 * s
        )
        self.action_instruct = tkinter.StringVar()
        self.action_label = tkinter.Label(
            self.action_frame,
            textvariable=self.action_instruct,
            font=("times", int(14 * s)),
            bg="black",
            fg="white",
        )
        self.action_label.pack()
        self.xscrollbar1 = tkinter.Scrollbar(
            self.action_frame, orient=tkinter.HORIZONTAL
        )
        self.yscrollbar1 = tkinter.Scrollbar(self.action_frame, orient=tkinter.VERTICAL)
        self.action_box = tkinter.Listbox(
            self.action_frame,
            name="actions:",
            selectmode="single",
            borderwidth=int(4 * s),
            xscrollcommand=self.xscrollbar1.set,
            yscrollcommand=self.yscrollbar1.set,
            font=("times", 12),
            bg="black",
            selectbackground="black",
            fg="white",
        )
        self.xscrollbar1.config(
            command=self.action_box.xview, troughcolor="white", bg="white"
        )
        self.xscrollbar1.pack(side=tkinter.BOTTOM, fill=tkinter.X)
        self.yscrollbar1.config(
            command=self.action_box.yview, troughcolor="white", bg="white"
        )
        ...
        self.xscrollbar2.config(
            command=self.move_box.xview, troughcolor="white", bg="white"
        )
        self.xscrollbar2.pack(side=tkinter.BOTTOM, fill=tkinter.X)
        self.yscrollbar2.config(
            command=self.move_box.yview, troughcolor="white", bg="white"
        )
    ...
    def user_input(self, actions):
        ...
        if selected_type in ["buy_available", "buy_reserve", "reserve"]:
            self.action_instruct.set(
                f'Select card to {"buy" if "buy" in selected_type else "reserve"}:'
            )
            for a in actions:
                action = a
        ...
        collect_combos = sorted(
            list(set([combo_to_str(a["collected_gems"]) for a in actions]))
        )
        ...
        for a in actions:
            if (
                combo_to_str(a["collected_gems"]) == collected_str
                and a["returned_gems"]
            ):
                any_returned = True
                break
        if any_returned:
            return_combos = sorted(
                list(
                    set(
                        combo_to_str(a["returned_gems"])
                        for a in actions
            ...
            for a in actions:
                if (
                    combo_to_str(a["collected_gems"]) == collected_str
                    and combo_to_str(a["returned_gems"]) == returned_str
                ):
                    return a
        else:
            for a in actions:
    ...
    def TimeOutWarning(self, runner, id):
        self._InsertState(
            "Agent {} time out, {} out of {}. Choosing random action instead.".format(
                id, runner.warnings[id], runner.warning_limit
            ),
            runner.game_rule.current_game_state,
        )
        if id == 0:
    ...
    def EndGame(self, game_state, scores):
        ...
        for i, plr_state in enumerate(game_state.agents):
            self._InsertState(
                "Final score for Agent {}: {}".format(i, plr_state.score), game_state
            )
...
class TextDisplayer(Displayer):
    ...
    def user_input(self, actions):
        ...
        try:
            ...
            user_input = input(
                "please input your choose between 0 and %d: " % (counter - 1)
            )
