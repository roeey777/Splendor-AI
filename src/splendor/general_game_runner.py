    ...
    if not is_git_repo(repo_path):
        ...
        try:
            repo = git.Repo.clone_from(
                clone_url, repo_path, branch=branch, no_checkout=True
            )
            repo.git.checkout(commit_id)
            # repo = git.Repo.clone_from(clone_url, repo_path)
            submission_time = get_commit_time(repo)
            logging.info(
                f"Team {team_name} cloned successfully with tag date {submission_time}."
            )
            ...
            logging.warning(
                f'Repo for team {team_name} with tag/branch "{branch}" cannot be cloned: {e.stderr.replace(token,"")}'
            )

            team_info.update({"git": "failed"})
            team_info.update(
                {
                    "comments": f'Repo for team {team_name} with tag/branch {branch} cannot be cloned: {e.stderr.replace(token,"")}'
                }
            )
            # repo.close()
            ...
            logging.warning(
                f"Repo for team {team_name} was cloned but has no tag {branch}, removing it...: {e}"
            )
            ...
            team_info.update(
                {
                    "comments": f"Repo for team {team_name} was cloned but has no tag {branch}, removing it...: {e}"
                }
            )
            # repo.close()
            ...
            team_info.update(
                {
                    "comments": f"Repo for team {team_name} cloned but unknown error when getting tag {branch}; should not happen. Stopping... {e}"
                }
            )
            # repo.close()
    else:
    ...
    if team_info["git"] == "succ":
        try:
            if not os.path.exists(f"agents/{team_name}"):
                shutil.copytree(
                    f"{repo_path}/agents/{team_name}", f"agents/{team_name}"
                )
        except:
            traceback.print_exc()
...
def loadAgent(matches, superQuiet=True):
    ...
    for i in range(num_of_agents):
        ...
        try:
            ...
            print(
                'Error: Agent at "' + teams[i]["agent"] + '" could not be loaded!',
                file=sys.stderr,
            )
            traceback.print_exc()
            pass
        ...
        if agent_temp != None:
            ...
            if not superQuiet:
                print(
                    "Agent {} team {} agent {} loaded".format(
                        i, matches["teams"][i]["team_name"], teams[i]["agent"]
                    )
                )
        else:
            valid_game = False
...
def run(options, msg):
    ...
    if options.replay != None:
        ...
        for game_num in range(options.multipleGames):
            ...
            f_name += "-" + str(
                random_seed
            )  # Add seed to replay filename for reproducibility.
            game.update({"file_name": f_name})
            if options.saveLog:
                game.update({"log_path": f"{file_path}/log-{f_name}.log"})
            gr = Game(
                GameRule,
                loaded_agents,
            ...
            if valid_game:
                ...
                scores, totals, wins, ties, loses = games_results[
                    len(games_results) - 1
                ]
                # print(games_results)
                new_scores = []
                ...
                ids, scores = list(
                    zip(
                        *sorted(
                            replay["scores"].items(), key=lambda x: x[1], reverse=True
                        )
                    )
                )
                ranks = []
                ...
                if not options.superQuiet:
                    print(
                        "Result of game ({}/{}):".format(
                            game_num + 1, options.multipleGames
                        )
                    )
                    for i in range(num_of_agents):
                        print(
                            "    {} earned {} points.".format(
                                agent_names[i], new_scores[i]
                            )
                        )

                games_results.append(
                    (new_scores, new_totals, new_wins, new_ties, new_loses)
                )
        ...
        if valid_game:
            ...
            if not options.superQuiet:
                ...
                for i in range(num_of_agents):
                    print(
                        "    {} earned {:.2f} on average and won {} games ({:.2f})%.".format(
                            agent_names[i], avgs[i], wins[i], win_rates[i]
                        )
                    )
...
def loadParameter():
    ...
    parser.add_option(
        ...
        default="splendor.agents.generic.random,splendor.agents.generic.random",
    )

    # parser.add_option('--redName', help='Red agent name', default='Red')
    # parser.add_option('--blueName', help='Blue agent name', default='Blue')
    parser.add_option(
        "--agent_names", help="A list of agent names", default="random0,random1"
    )
    ...
    parser.add_option(
        ...
        "--num_of_agents",
        type="int",
        help="The number of agents in this game",
        default=2,
    )
    ...
    parser.add_option(
        "-g",
        "--game",
        help="The name of the game, starting with a uppercase character (default: Splendor)",
        default="Splendor",
    )
    parser.add_option(
        "-q",
        "--quiet",
        action="store_true",
        help="No text nor graphics output, only show game info",
        default=False,
    )
    parser.add_option(
        "-Q",
    ...
    parser.add_option(
        ...
        "--warningTimeLimit",
        type="float",
        help="Time limit for a warning of one move in seconds (default: 1)",
        default=1.0,
    )
    parser.add_option(
        "--startRoundWarningTimeLimit",
        type="float",
        help="Time limit for a warning of initialization for each round in seconds (default: 5)",
        default=5.0,
    )
    parser.add_option(
        "--numOfWarnings",
        type="int",
        help="Num of warnings a team can get before fail (default: 3)",
        default=3,
    )
    parser.add_option(
        "-m",
        "--multipleGames",
        type="int",
        help="Run multiple games in a roll",
        default=1,
    )
    parser.add_option(
        "--setRandomSeed",
    ...
    parser.add_option(
        ...
        "--saveGameRecord",
        action="store_true",
        help="Writes game histories to a file (named by teams' names and the time they were played) (default: False)",
        default=False,
    )
    parser.add_option(
        "-o",
        "--output",
        help="output directory for replay and log (default: output)",
        default="output",
    )
    parser.add_option(
        "-l",
        "--saveLog",
        action="store_true",
        help="Writes agent printed information into a log file(named by the time they were played)",
        default=False,
    )
    parser.add_option(
        "--replay", default=None, help="Replays a recorded game file by a relative path"
    )
    parser.add_option(
        "--delay",
        type="float",
        help="Delay action in a play or replay by input (float) seconds (default 0.1)",
        default=0.1,
    )
    parser.add_option(
        "-p",
        "--print",
        action="store_true",
        help="Print all the output in terminal when playing games, will diable '-l' automatically. (default: False)",
        default=False,
    )
    parser.add_option(
        "--half-scale",
        action="store_true",
        help="Display game at half-scale (default is 1920x1080)",
        default=False,
    )
    parser.add_option(
        "--interactive",
