    ...
    def SelectAction(self, actions, rootstate):
        ...
        while len(queue) and time.time() - start_time < THINKTIME:
            (
                state,
                path,
            ) = queue.popleft()  # Pop the next node (state, path) in the queue.
            new_actions = GetActions(
                state
            ...
            for a in new_actions:  # Then, for each of these actions...
                ...
                if reward:
                    return next_path[
                        0
                    ]  # If this action resulted in a reward, return the initial action that led there.
                else:
                    queue.append(
                        (next_state, next_path)
                    )  # If no reward, simply add this state and its path to the queue.

        return actions[
            0
        ]  # If no reward was found in the time limit, return the first available action.
