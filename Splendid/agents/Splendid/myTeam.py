...
STARTUP_TIME = 4  # SECONDS
ROUND_TIME = 1  # SECOND
SEARCH_DEPTH = 7  # MOVES
ROUND_END_BUFFER = 0.018  # Seconds from end of round. Needed because simulate() doesn't keep track of time(only search depth) right now and mcts() may go over time as a result
...
class myAgent(Agent):
    ...
    def SelectAction(self, actions, game_state):
        ...
        try:
            ...
            # profiler.enable()
            action = self.player.mcts(
                game_state, ROUND_TIME + STARTUP_TIME - ROUND_END_BUFFER
            ).getBestAction()
            # profiler.disable()
        ...
        if match:
            ...
            for action in actions:
                if "buy" in action["type"]:
                    if action["card"].points > highest:
                        highest = action["card"].points
                        best = action
            return best
...
def equalStates(state1: SplendorState, state2: SplendorState):
    ...
    if aid1 != aid2:
        return False  # Check same agent to move
    if state1.agents[aid1].last_action != state2.agents[aid2].last_action:
        return False  # Check last actions equal
    for colour in agent1.gems:  # Check gems are the same
        if agent1.gems[colour] != agent2.gems[colour]:
            return False
    if str(state1.board.dealt) != str(state2.board.dealt):
        return False  # Check cards dealt are the same
    return True
...
class Splendor(MDP):
    ...
    def getTransitions(self, state, action, new=True):  # Jesus
        if new:
                ...
                if action["type"] == "buy_reserve":
                    for i in range(len(state.agents[aid].cards["yellow"])):
                        if state.agents[aid].cards["yellow"][i].code == boughtCard.code:
                            nextState.agents[aid].cards[boughtCard.colour].append(
                                boughtCard
                            )
    ...
    def execute(self, state, action, copy=True):
        nextState, distr = self.getTransitions(state, action, copy)[0]
    ...
    def getReward(self, state, action, nextState):  # Allison
        ...
        if (
            nextState.agents[aid].score >= 15
            and nextState.agents[aid].score >= opp.score
        ):  # Winning is ultimate reward
            reward_point += 9999  # + nextState.agents[aid].score/sum(map(len,nextState.agents[aid].cards))
        if "buy" in action["type"]:
            card = action["card"]
            cost = sum(action["returned_gems"].values())
            reward_point += card.points / (
                cost + 5
            )  # Reward development based on point to cost ratio
            for (
                noble
            ) in (
                state.board.nobles
            ):  # More points if bought development relevant to nobles
                if (
                    card.colour in noble[1]
                    and len(agent.cards[card.colour]) < noble[1][card.colour]
                ):
                    reward_point += card.points / (cost + 1) + 1
        ...
        if "collected_gems" in action:
            if len(action["collected_gems"]) > 1:
                for colour in action[
                    "collected_gems"
                ]:  # More points if get gems relevant to cards dealt
                    for row in nextState.board.dealt + [agent.cards["yellow"]]:
                        for card in row:
                            if (
                                ...
                                < card.cost[colour]
                            ):  # Reward picking gems if it can be used next buy
                                cost = 0
                                for colour in card.cost:
                                    cost += max(
                                        0, card.cost[colour] - len(agent.cards[colour])
                                    )
                                reward_point += (
                                    card.points + 0.05
                                ) / cost  # Rewards for gems for each development with more reward if closer to buying
                                for (
                                    noble
                                ) in (
                                    state.board.nobles
                                    ...
                                    if (
                                        ...
                                        reward_point += (card.points + 0.1) / cost
            else:
                reward_point -= 0.5 * len(
                    action["returned_gems"]
                )  # Less rewards for returning gems
        ...
        if action["type"] == "reserve":  # Reward for reserving cards.
            ...
            for colour in card.cost:
                gems1 += max(
                    0, card.cost[colour] - len(agent.cards[colour]) - agent.gems[colour]
                )
                gems2 += max(
                    0,
                    card.cost[colour]
                    - len(opponent.cards[colour])
                    - opponent.gems[colour],
                )
            reward_point += (card.points) / (gems2 + 50) + (card.points) / (gems1 + 150)
    ...
    def isTerminal(self, state):
        for agent in state.agents:
            if (
                agent.score >= 15 and state.agent_to_move == 0
            ):  # Game is over after last player's action and a player is over 15 points
                return True
        return False
...
class UpperConfidenceBounds(MultiArmedBandit):
    ...
    def select(self, actions, qValues):
        ...
        for action in actions:
            N = self.N[action]
            value = qValues[action] + 2 * math.sqrt(
                (2 * math.log(self.total)) / (2 * N)
            )
            if value > maxValue:
                maxActions = [action]
                maxValue = value
...
class StateNode(Node):
    ...
    def expand(self):
        ...
        unexpanded_action_strings = (
            action_strings - self.children.keys()
        )  # NOTE: it's something about dictionary keys needing to be immutable. action dictionaries are mutable
        # Retrieve corresponding action dictionaries
        actions = [
            action for action in actions if str(action) in unexpanded_action_strings
        ]
...
class EnvironmentNode(Node):
    ...
    def backPropagate(self, reward):
        ...
        if self.children[0].value > self.value:
...
class MCTS:
    ...
    def mcts(self, currentState, timeout):
        ...
        #     if 'buy' in k or 'reserve' in k: print(k,":", v.getValue(), ";",v.children[0].reward, v.children[0].value,";", v.state.agents[self.mdp.id].score, "->", v.children[0].state.agents[self.mdp.id].score)
        return rootNode
    ...
    def heuristic(self, action, state):
        ...
        if action["noble"]:
            noble = 5
        value = 2 * (agent.score - opp.score + 20 + noble)
        if agent.score + noble >= 15:
            return value + 50
        if "buy" in action["type"]:
            card = action["card"]
            if agent.score + card.points + noble >= 15:
                return value + 50
            cost = len(action["returned_gems"])
            value += (card.points + 1) / (cost + 1)
            for _, noble in state.board.nobles:
                if (
                    card.colour in noble
                    and len(agent.cards[card.colour]) < noble[card.colour]
                ):
                    value += 1 / (cost + 1)
        if action["type"] == "reserve":
            ...
            cost = sum(
                [
                    max(0, card.cost[colour] - len(agent.cards[colour]))
                    for colour in card.cost
                ]
            )
            value += (card.points) / (cost + 1)
        if "collected_gems" in action:
            value += 1

        return min(29, value / len(agent.cards))
    ...
    def simulate(self, node):
        ...
        depth = 0
        while depth < SEARCH_DEPTH and not self.mdp.isTerminal(
            state
        ):  # NOTE: SAME AS BEFORE. STOP EARLY
            # choose an action to execute
            action = self.choose(state)

            # execute the action
            (newState, reward) = self.mdp.execute(state, action, False)
            # print("Reward:", reward)
            # discount the reward
...
class SplendorGameRule2(SplendorGameRule):
    ...
    def getLegalActions(self, game_state, agent_id):
        ...
        available_colours = [
            ...
            if colour != "yellow" and number > 0 and can_take > 1
        ]
        ...
        available_colours = [
            colour
            for colour, number in board.gems.items()
            if colour != "yellow" and number >= 4 and can_take > 0
        ]
        ...
        if len(agent.cards["yellow"]) < 3:
            ...
            for returned_gems in return_combos:
                for card in board.dealt_list():
                    if card and (
                        (card.points) / (sum(card.cost.values())) > 0.24
                        or card.points + opp.score >= 15
                    ):
                        c1 = c2 = 0
                        for colour in card.cost:
                            c1 += max(
                                0,
                                card.cost[colour]
                                - len(agent.cards[colour])
                                - agent.gems[colour],
                            )
                            c2 += max(
                                0,
                                card.cost[colour]
                                - len(opp.cards[colour])
                                - opp.gems[colour],
                            )
                        deck = card.deck_id
                        if (
                            c1 == 0 or (c1 > 1 and c2 > 0) or deck < 1
                        ):  # (opp.score>0 and agent.score>0 and deck==0) or (opp.score==0 and agent.score==0 and deck>1):
                            if len(actions) > 0:
                                continue
                        for noble in potential_nobles:
                            actions.append(
