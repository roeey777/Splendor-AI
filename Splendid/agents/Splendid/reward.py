...
class Splendor(MDP):
    ...
    def getReward(self, state, action, nextState):  # Allison
        ...
        if "buy" in action["type"]:
            ...
            sameType = len(agent.cards[card.colour])
            reward_point += card.points - cost - sameType
            for (
                noble
            ) in state.board.nobles:  # More points if development relevant to nobles
                if card.colour in noble[1] and sameType < noble[1][card.colour]:
                    reward_point += card.points / (0.1 + cost + sameType) + 6
        if action["noble"]:
            reward_point += 10 * len(action["noble"])
        if "collected_gems" in action:
            if len(action["collected_gems"]) > 1:
                for colour in action[
                    ...
                    count = action["collected_gems"][colour]
                ...
                reward_point -= 0.5 * len(action["returned_gems"])

        if action["type"] == "reserve":
            card = action["card"]
            reward_point += card.points / sum(card.cost.values())
...
class SplendorGameRule2(SplendorGameRule):
    ...
    def getLegalActions(self, game_state, agent_id):
        ...
        if len(agent.cards["yellow"]) < 3 and len(actions) < 2:
