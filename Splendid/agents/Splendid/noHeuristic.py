...
ROUND_END_BUFFER = 0.02  # Seconds from end of round. Needed because simulate() doesn't keep track of time(only search depth) right now and mcts() may go over time as a result
    ...
    def SelectAction(self, actions, game_state):
        ...
        try:
            ...
            action = self.player.mcts(
                game_state, ROUND_TIME - ROUND_END_BUFFER
            ).getBestAction()
            # profiler.disable()
...
class Splendor(MDP):
    ...
    def getReward(self, state, action, nextState):  # Allison
        ...
        if "buy" in action["type"]:
            card = action["card"]
            cost = sum(card.cost.values()) + len(card.cost)
            reward_point += (
                2 * card.points
            )  # - (len(agent.cards[card.colour])**2 + cost)/(card.deck_id+1)
            for (
                noble
            ) in state.board.nobles:  # More points if development relevant to nobles
                if (
                    card.colour in noble[1]
                    and len(agent.cards[card.colour]) < noble[1][card.colour]
                ):
                    reward_point += card.points / cost + 25
        if action["noble"]:
            reward_point += 15 * len(action["noble"])
                    ...
                    for row in nextState.board.dealt + [agent.cards["yellow"]]:
                        for card in row:
                            if (
                                ...
                                cost = sum(card.cost.values()) + len(card.cost)
                                reward_point += (card.points + 1) / (
                                    cost - agent.gems[colour] - len(agent.cards[colour])
                                )
                                for (
                                    noble
                                ) in (
                                    ...
                                    if (
                                        ...
                                        reward_point += (card.points + 2) / (
                                            cost
                                            + noble[1][card.colour]
                                            - len(agent.cards[card.colour])
                                            - card.deck_id
                                        )
            else:
        ...
        if action["type"] == "reserve":
            card = action["card"]
            reward_point += (card.points + card.deck_id) / sum(card.cost.values())
...
class SplendorGameRule2(SplendorGameRule):
    ...
    def getLegalActions(self, game_state, agent_id):
        ...
        if len(agent.cards["yellow"]) < 3 and len(actions) < 3:
